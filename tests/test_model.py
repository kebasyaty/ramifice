...
class TestModel(unittest.TestCase):
    ...
    def test_instance_model(self):
        ...
        self.assertEqual(m.id__attrs.label, "Идентификатор документа")
        self.assertEqual(m.id__attrs.placeholder, "Он добавляется автоматически")
        self.assertEqual(m.id__attrs.hint, "Он добавляется автоматически")
        self.assertEqual(len(m.id__attrs.warning), 0)
        self.assertTrue(m.id__attrs.hide)
        self.assertTrue(m.id__attrs.disabled)

        self.assertEqual(m.created_at__attrs.label, "Создан")
        self.assertEqual(m.created_at__attrs.placeholder, "Он добавляется автоматически")
        self.assertEqual(m.created_at__attrs.hint, "Он добавляется автоматически")
        self.assertEqual(m.created_at__attrs.warning[0], "Когда был создан документ.")
        self.assertTrue(m.created_at__attrs.hide)
        self.assertTrue(m.created_at__attrs.disabled)

        self.assertEqual(m.updated_at__attrs.label, "Обновлен")
        self.assertEqual(m.updated_at__attrs.placeholder, "Он добавляется автоматически")
        self.assertEqual(m.updated_at__attrs.hint, "Он добавляется автоматически")
        self.assertEqual(m.updated_at__attrs.warning[0], "Когда был обновлен документ.")
        self.assertTrue(m.updated_at__attrs.hide)
        self.assertTrue(m.updated_at__attrs.disabled)
