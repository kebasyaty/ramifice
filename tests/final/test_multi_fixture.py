"""Testing the `multi_fixture` example."""
...
@meta(
    service_name="Admin",
    fixture_name="SiteParameters_2",  # config/fixtures/SiteParameters_2.yml
...
class Testmulti_fixtureExample(unittest.IsolatedAsyncioTestCase):
    """Testing the `multi_fixture` example."""

    async def test_multi_fixture_example(self):
        """Testing the `multi_fixture` example."""
        # Maximum number of characters 60.
        database_name = "multi_fixture_example"
        ...
        self.assertEqual(await SiteParameters.estimated_document_count(), 2)
        ...
        # Get Site Parameters 2
        site_params: SiteParameters | None = await SiteParameters.find_one_to_instance_model({"brand": "Brand Name 2"})
        self.assertIsNotNone(site_params)
        self.assertTrue(isinstance(site_params.logo, dict))
        self.assertTrue(isinstance(site_params.copyright, dict))
        self.assertEqual(site_params.brand, "Brand Name 2")
        self.assertEqual(site_params.slogan, "We are the best!")
        self.assertIsNone(site_params.about_site)
        self.assertEqual(site_params.email_feedback, "John_Smith@gmail.com")
        self.assertEqual(site_params.start_date, date(2000, 1, 25))
