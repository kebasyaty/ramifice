...
class IndexMixin:
	...
	async def create_index(cls, keys, session=None, comment=None, **kwargs) -> str:
		...
		# Create index.
		result: str = await collection.create_index(keys=keys, session=session, comment=comment, **kwargs)
		return result
	...
	async def drop_index(cls, index_or_name, session=None, comment=None, **kwargs) -> None:
		...
		collection: AsyncCollection = store.MONGO_DATABASE[cls.META["collection_name"]]
		# Delete index.
		await collection.drop_index(index_or_name=index_or_name, session=session, comment=comment, **kwargs)
	...
	async def create_indexes(cls, indexes, session=None, comment=None, **kwargs) -> list[str]:
		...
		collection: AsyncCollection = store.MONGO_DATABASE[cls.META["collection_name"]]
		# Create indexes.
		result: list[str] = await collection.create_indexes(indexes=indexes, session=session, comment=comment, **kwargs)
		return result
